...
interface CalCell { day: number; current: boolean; isToday: boolean; hasSession: boolean; }
...
export class DashboardPageComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.todayLabel = new Date().toLocaleDateString('fr-FR', { weekday: 'long', day: 'numeric', month: 'long' });
  ...
  get completedCount(): number { return this.sessions.filter((s) => s.status === 'completed').length; }
  ...
  get weeklyHours(): number { return this.weekly.reduce((s, p) => s + p.hours, 0); }
  get todaySessions(): number {
    const today = new Date().toDateString();
    return this.sessions.filter((s) => new Date(s.planned_start).toDateString() === today).length;
  }
  get unreadCount(): number { return this.notifications.filter((n) => !n.is_read).length; }
  get monthLabel(): string {
    return new Date(this.calYear, this.calMonth).toLocaleDateString('fr-FR', { month: 'long', year: 'numeric' });
  }
  ...
  private buildCalendar(): void {
    ...
    const sessionDates = new Set(this.sessions.map((s) => new Date(s.planned_start).getDate() + '-' + new Date(s.planned_start).getMonth()));
    for (let d = 1; d <= last.getDate(); d++) {
      const isToday = d === today.getDate() && this.calMonth === today.getMonth() && this.calYear === today.getFullYear();
      const key = `${d}-${this.calMonth}`;
      cells.push({ day: d, current: true, isToday, hasSession: sessionDates.has(key) });
    }
    let nd = 1;
    while (cells.length % 7 !== 0) { cells.push({ day: nd++, current: false, isToday: false, hasSession: false }); }
    this.calCells = cells;
  }
  ...
  sessionDuration(s: StudySession): string {
    const mins = Math.round((new Date(s.planned_end).getTime() - new Date(s.planned_start).getTime()) / 60000);
    if (mins >= 60) return `${Math.floor(mins / 60)}h${mins % 60 ? (mins % 60) + 'min' : ''}`;
    return `${mins}min`;
  }

  statusChip(status: string): string {
    return { completed: 'chip-done', in_progress: 'chip-live', planned: 'chip-soon' }[status] ?? 'chip-soon';
  }
